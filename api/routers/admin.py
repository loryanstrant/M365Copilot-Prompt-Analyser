...
from worker.analysis import AnalysisError, run_analysis, test_aoai_connection
...
async def put_config(
    ...
    if body.aoai_endpoint is not None:
    ...
    if body.analysis_mode is not None:
        cfg.analysis_mode = body.analysis_mode
...
@router.post("/test-aoai", response_model=TestAoaiOut)
async def test_aoai(
    session: AsyncSession = Depends(get_session),
) -> TestAoaiOut:
    cfg = await _get_config(session)
    if cfg is None:
        return TestAoaiOut(ok=False, detail="Azure OpenAI is not configured yet.")
    result = await test_aoai_connection(cfg)
    return TestAoaiOut(**result)


async def _run_manual_analysis() -> None:
    async with _analysis_lock:
        try:
            await run_analysis(SessionLocal, job_name="manual-analysis")
        except AnalysisError as exc:
            logger.info("Manual analysis skipped: %s", exc)
        except Exception:  # pragma: no cover - logged for observability
            logger.exception("Manual analysis failed")


@router.post("/analysis/run", response_model=IngestRunOut)
async def analysis_run(background: BackgroundTasks) -> IngestRunOut:
    if _analysis_lock.locked():
        return IngestRunOut(
            status="already_running", detail="An analysis run is already in progress."
        )
    background.add_task(_run_manual_analysis)
    return IngestRunOut(status="started", detail="Analysis started in the background.")
