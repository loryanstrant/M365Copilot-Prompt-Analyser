...
class Prompt(Base):
    ...
    # The human prompt text — the substance this whole solution analyses.
    prompt_text: Mapped[str | None] = mapped_column(Text)
    # Populated by the sensitivity stage (1-10 each), null until analysed.
    name_confidence: Mapped[int | None] = mapped_column(Integer)
    sensitive_confidence: Mapped[int | None] = mapped_column(Integer)
    curse_confidence: Mapped[int | None] = mapped_column(Integer)
    # True once this prompt has been through LLM analysis (drives incremental work).
...
class PromptAnalysis(Base):
    """Per-prompt LLM analysis (quality / GCSE / sentiment / category)."""

    __tablename__ = "prompt_analysis"

    prompt_id: Mapped[str] = mapped_column(Text, primary_key=True)
    conversation_id: Mapped[str | None] = mapped_column(Text, index=True)
    user_generated: Mapped[bool | None] = mapped_column(Boolean)
    sentiment: Mapped[str | None] = mapped_column(Text)
    quality_score: Mapped[int | None] = mapped_column(Integer)
    quality_rationale: Mapped[str | None] = mapped_column(Text)
    category: Mapped[str | None] = mapped_column(Text)
    gcse_goal: Mapped[int | None] = mapped_column(Integer)
    gcse_context: Mapped[int | None] = mapped_column(Integer)
    gcse_source: Mapped[int | None] = mapped_column(Integer)
    gcse_expectation: Mapped[int | None] = mapped_column(Integer)
...
class ConversationAnalysis(Base):
    """Per-conversation LLM analysis (one row per Copilot session)."""

    __tablename__ = "conversation_analysis"

    conversation_id: Mapped[str] = mapped_column(Text, primary_key=True)
    sentiment: Mapped[str | None] = mapped_column(Text)
    avg_quality_score: Mapped[float | None] = mapped_column(Numeric(4, 1))
    conversation_quality_score: Mapped[int | None] = mapped_column(Integer)
    user_generated_ratio: Mapped[float | None] = mapped_column(Float)
    ...
    suggested_starter_prompt: Mapped[str | None] = mapped_column(Text)
...
class AppConfig(Base):
    ...
    aoai_api_version: Mapped[str | None] = mapped_column(
        Text, default="2025-01-01-preview"
    )
    aoai_key_encrypted: Mapped[str | None] = mapped_column(Text)
    # "combined" = one call per conversation (default); "split" = analyser per
    # conversation + sensitivity per prompt (matches the original solution).
    analysis_mode: Mapped[str] = mapped_column(Text, default="combined")
